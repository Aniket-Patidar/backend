...
const generateBill = (user: any,_id:string,shippingAddress:string,TodayDate:string,totalPrice:string) => {  
  return `
...
export const order = async (
  ...
  const orderID = await instance.orders.create(options, function (err, order) {
    console.log(order);
  });
...
export const ValidateOrder = catchAsyncError(
  async (req: CartItemRequest, res: Response, next) => {
    try {
      const { razorpay_order_id, razorpay_payment_id, razorpay_signature } =
        req.body;

      const sha = crypto.createHmac("sha256", "7XWnFM3iB3hdN7IJHlKbgu0e");
